package com.linecy.dilidili.ui.widget;
...
public class RollSquareView extends View {
  ...
  private void initSquares(int startEmptyPosition) {
    ...
    for (int i = 0; i < mFixSquares.length; i++) {
      ...
      mFixSquares[i].isShow = startEmptyPosition != i;
  ...
  private void fixRollSquarePosition(FixSquare[] fixSquares, RollSquare rollSquare,
      int startEmptyPosition, boolean isClockwise) {
   ...
   * 固定这些图形的rect位置
   */
  private void fixFixSquarePosition(FixSquare[] fixSquares, int cx, int cy, float dividerWidth,
    ...
    for (int i = 0; i < lineCount; i++) {//行
      for (int j = 0; j < lineCount; j++) {//列
        if (i == 0) {
          if (j == 0) {
            fixSquares[0].rectF.set(firstRectLeft, firstRectTop, firstRectLeft + squareWidth,
  ...
  private ValueAnimator createTranslateValueAnimator(FixSquare currEmptyFixSquare,
    ...
    PropertyValuesHolder left;
    PropertyValuesHolder top;
